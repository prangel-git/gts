    ...
    T: Environment<Action, AgentId> + Copy + Eq + Hash,
{
    ...
    if actions.is_empty() {
        ...
        let action_wins_loses = 
        actions
        .iter()
        .map(|x| (*x, read_cache(&env.what_if(x), cache)))
        .collect::<Vec<_>>();

        let total_simulations = action_wins_loses.iter().fold(0u32, |a,(_,(w,l))| a + w + l) as f64;
        ...
        let (best_action, _) = if is_agent_turn {
            ...
            .fold(
                ...
                |(act_0, a_0), (act_1, a_1)| if a_0 < a_1 { (*act_1, a_1) } else {(act_0, a_0)}
            )
        } else {
            action_wins_loses
            .iter()
